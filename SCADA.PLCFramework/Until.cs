...
namespace SCADA.PLCFramework
{
    internal static class Until
    {
        ...
        public static void ValidatePlcInfoContent(string content)
        {
            ...
            var plcinfoAttrs = plcinfoNode.Attributes;
            plcinfoAttrs.Cast<XmlElement>().ToList().ForEach(attr =>
            {
                var attrName = attr.Name.Trim();
                if (attrName != "module" && attrName != "name" && attrName != "buffcount" && attrName != "address" && attrName != "class" && attrName != "assembly")
                {
                    throw new FormatException($"Invalid attribute name {attr.Name} in 'PlcInfo' element.Hints:{plcinfoNode.OuterXml}.");
                }
            });
            var moduleAttr = plcinfoAttrs["module"];
            if (moduleAttr == null)
            {
                throw new FormatException($"Missing required 'module' attribute in 'PlcInfo' element.Hints:{plcinfoNode.OuterXml}.");
            }
            else
            {
                string moduleAttrValue = moduleAttr.Value.Trim();
                if (!string.IsNullOrWhiteSpace(moduleAttrValue))
                {
            ...
            var buffcount = plcinfoNode.Attributes["buffcount"];
            if (buffcount == null)
                throw new FormatException($"Invalid PLC info content: Missing required 'buffcount' attribute in 'PlcInfo' element.Hints:'{plcinfoNode.OuterXml}'.");
            if (!int.TryParse(buffcount.Value.Trim(), out int buffcountNum) || buffcountNum <= 4 || buffcountNum > 100)
            ...
            // 节点只能是 BlockList IOList Interlock
            plcinfoNode.ChildNodes.Cast<XmlNode>().ToList().ForEach(node =>
            {
                if (node.Name.Trim() != "BlockList" && node.Name.Trim() != "IOList" && node.Name.Trim() != "Interlock")
