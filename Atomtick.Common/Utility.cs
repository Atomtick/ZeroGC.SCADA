...
namespace SCADA.Common
{
    public static class Utility
    {
        ...
        public static void SetBitValue(byte data, int index, bool value)
        {
            if (index < 0 || index > 7)
            {
                throw new ArgumentOutOfRangeException(nameof(index));
            }
            data = value ? (byte)(data | (1 << index)) : (byte)(data & ~(1 << index));
        }

        public static void ToggleBitValue(byte data, int index)
        {
            if (index < 0 || index > 7)
            {
                throw new ArgumentOutOfRangeException(nameof(index));
