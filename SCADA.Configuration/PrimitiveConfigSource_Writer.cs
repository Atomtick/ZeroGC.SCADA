...
namespace SCADA.Configuration
{
    public partial class PrimitiveConfigSource : IConfigWriter
    {
        ...
        public IConfigWriter Set(long transactionId, string config, object value)
        {
            ...
            if (configItems.ContainsKey(config) == false)
            {
        ...
        private List<string> EqualsKeys = new List<string>();

        private void Set(LightWeightDictionary modfificationConfigsDict)
        {
            if (modfificationConfigsDict == null || modfificationConfigsDict.Count() == 0)
            {
                return;
            }
            ...
            EqualsKeys.Clear();
            foreach (var pair in modfificationConfigsDict)
            {
                if ((pair.Value as string) == _configItems[pair.Key].StringValue)
                {
            ...
            {
                modfificationConfigsDict.Remove(item);
            }

            EqualsKeys.Clear();

            if (modfificationConfigsDict.Count() > 0)
            {
                foreach (var pair in modfificationConfigsDict)
                {
            ...
            {
                // 异步刷盘
                if (_channel.Writer.TryWrite(modfificationConfigsDict) == false)
                {
                    throw new Exception("Failed to add to the asynchronous persistence to disk queue.");
                }
                // 触发事件,一般用于日志记录
                ValueSet?.Invoke(modfificationConfigsDict);
            }
        }
    }
}